...
if(isset($request['params']['id'])   && $request['params']['id']>0)
    ...
    informError(true,make_url("Entrada","index"));
...
<div id="main" role="main">
    <?php $breadcrumbs["Entradas"] = APP_URL."/Entradas"; include(SYSTEM_DIR . "/inc/ribbon.php"); ?>
        ...
        <section id="widget-grid" class="">
			<div class="row">   
                <div class="widget-body" style='padding-left: 15px;'>
					<a class="btn btn-success" target="_blank" href="<?php echo make_url("Entradas","print",array('id'=>$id,'page'=>'entrada'))?>" ><i class="fa fa-print"></i> &nbsp;Imprimir</a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        validar = function(id_entrada){
            var url = config.base+"/Entradas/ajax/?action=get&object=validar"; 
            $.ajax({
                type: "POST",
                url: url,
        ...
        showpopuppagar = function(id_entrada){
            $('#titlemodal').html('<span class="widget-icon"><i class="far fa-plus"></i> Nuevo Pago</span>');
            $.get(config.base+"/Entradas/ajax/?action=get&object=showpopuppagar&id="+id_Entrada, null, function (response) {
                    if ( response ){
        ...
        $('body').on('click', '#savenewpago', function(){
            ...
            var url = config.base+"/Entradas/ajax/?action=get&object=savenewpago"; 
            $.ajax({
                type: "POST",
                url: url,
