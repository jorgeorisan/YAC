...
class Venta extends AutoVenta { 
	...
	public function getReporteComisionesUsuarios($arrayfilters)
	{
		...
		$qrytienda      = ($id_tienda>0) ? " AND v.id_tienda  = '$id_tienda'  " : "";
		$sql = "SELECT 
						usuarios_venta.id_usuario id_usuario,usuarios_venta.comision,
					...
					FROM (
						SELECT SUM(pv.total) as totalventa,SUM(v.descuento) totalventadescuento, v.id_user , u.comision,u.id_usuario
					...
					LEFT JOIN (
						SELECT SUM(pv.total) as totalventacredito, v.id_user id_user 
						FROM  productos_venta pv
						INNER JOIN venta v ON pv.id_venta=v.id_venta
						where  
							DATE(v.fecha)>='".$fechaini."' and DATE(v.fecha)<='".$fechafin."'
