	...
	// Public Support Functions
		...
		public function save() {
			if ($this->getId()==0){ // insert new
				$sql = "INSERT INTO tienda SET modified=UTC_TIMESTAMP(),created=UTC_TIMESTAMP(),"; 

			$sql = trim($sql,",");

			} else { // updated existing
				$sql = "UPDATE tienda SET modified=UTC_TIMESTAMP(),";	
		...
		public function getAll() {
			$sql="SELECT id FROM tienda WHERE 1 and status='active'";
			// Get data 
			$stmt = $this->db->prepare( $sql );
			$stmt->execute();
