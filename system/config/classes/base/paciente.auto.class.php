	...
	// Public Support Functions
		...
		public function save() {
			if ($this->getId()==0){ // insert new
				$sql = "INSERT INTO paciente SET modified=UTC_TIMESTAMP(),created=UTC_TIMESTAMP(),"; 
			...
			$sql = trim($sql,",");
				...
				$sql = "UPDATE paciente SET modified=UTC_TIMESTAMP(),";	
			...
			$stmt->mbind_param( 's', $this->alergias );
			if ($this->getId()>0){
		...
		public function updateFields($fieldstoupdate) {
			...
			if (in_array("alergias",$fieldstoupdate)){
				$sql .= " `alergias` = ? ,";
			}
			$sql = trim($sql,",");
			$sql .= " WHERE id = ?";
		...
		public function getAll() {
			$sql="SELECT id FROM paciente WHERE 1 and status='active'";
			// Get data 
			$stmt = $this->db->prepare( $sql );
			$stmt->execute();
