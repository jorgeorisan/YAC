...
class Administracion_ProductosController extends jfLib_Controller
{
               ...
               // var_dump($_FILES['imagen']);
                ...
                if($codbar==""){
                    try {
                        //var_dump($_FILES['imagen']);
                         $hora=date('Ymd_h:i:s');
                         $obj->imagen=$hora.$_FILES['imagen']['name'];
                        if(copy($_FILES['imagen']['tmp_name'],"./images/imagenesproductos/".$hora.$_FILES['imagen']['name'])){
        ...
        if($id=$this->_request->getParam("id2")){
            $query = Doctrine_Query::create()
                ...
                ->where("id_producto = ?", $id)
                  ->andwhere("t.tienda_id_tienda=13")
                ->fetchOne();
        }
    ...
    function excelproductoAction(){
        ...
        $xlsName = "Reporte de Todo el Inventario ".date("Y-m-d_H-i-s") . ".xlsx";
        $fileName =  $xlsName;
        // echo $fileName = "http://phpstack-1171-4157-7368.cloudwaysapps.com/puntoventacremas/public/sistema/documentos/" . $xlsName;
